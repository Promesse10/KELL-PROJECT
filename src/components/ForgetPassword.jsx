...
function ForgotPasswordForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(t('forgotPassword.resetError', { error: err.message || t('forgotPassword.resetErrorDefault') }));
    }
  };
  ...
  return (
    <div className="flex flex-col justify-center items-center min-h-screen bg-gray-100">
      <div className="flex flex-col w-full max-w-md p-6 bg-white rounded-lg shadow-md">
        <h1 className="text-2xl font-semibold text-center mb-4">{t('forgotPassword.title')}</h1>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <input
              type="email"
          ...
          <div className="flex justify-center">
            <button
              ...
              className="w-full p-3 text-white bg-blue-600 rounded-lg focus:outline-none focus:ring focus:ring-blue-200"
            >
              {loading ? t('forgotPassword.processingButton') : t('forgotPassword.resetPasswordButton')}
            </button>
          </div>
          {error && <p className="text-red-500 text-center">{error.message}</p>}
        </form>
